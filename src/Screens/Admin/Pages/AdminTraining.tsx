...
import { img, sectionSchema, trainingPage } from "../../../Shared/types";
...
function AdminTraining() {
	const { data: trainingPage } = useFetchData<trainingPage[]>("/training");
	...
	useEffect(() => {
		if (trainingPage && trainingPage.length !== 0) {
			setHeaderText(trainingPage[0].title);
			setEditorText(`<p>${trainingPage[0].subtitle}</p>`);
			setSections(
				trainingPage[0].sections.map((el: sectionSchema, index: number) => ({
					...el,
					id: index.toString(),
		...
		<div>
			...
			<div className="w-full ">
				<div className="w-full h-full p-3 bg-white ">
					...
					{/* Sections */}
					<p className="my-2 font-medium text-gray-700 ">Cards</p>

					{sections &&
						sections.map((section: editableSection, index: number) => (
							<div className="flex gap-4 my-3 h-[30vh] " key={index}>
								<div className="w-1/2">
									<div className="relative">
										...
										<div className="h-full">
											<div>
												...
												<input
													...
													type="text"
													className="w-full px-3 py-1 mb-2 border border-gray-500 rounded shadow-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
												/>
											</div>
			...
			{trainingPage && trainingPage.length !== 0 ? (
				<UpdateItemModal
					setShow={setShow}
					data={{
						title: headerText,
